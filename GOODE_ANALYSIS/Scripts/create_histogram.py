...
def create_histogram():
    ...
    try:
        ...
        if filtered_frequency and bin_labels and bin_midpoints and filtered_normal:
            log_message("All necessary data ranges are available.", log_num)
            ...
            y_smooth_actual = np.interp(x_smooth, np.arange(len(bin_labels)), filtered_frequency)
            plt.plot(x_smooth, y_smooth_actual, color='blue', label='Actual Distribution')

            # Plot Expected Distribution line chart
            y_smooth_expected = np.interp(x_smooth, np.arange(len(bin_labels)), filtered_normal)
            plt.plot(x_smooth, y_smooth_expected, color='orange', linestyle='-', label='Expected Distribution')
            ...
            # Plot Mean line
            mean_tick = np.argmin(np.abs(np.array(bin_midpoints) - mean_value))
            plt.axvline(mean_tick, color='red', linestyle='--', label='Mean')
